...
<<<<<<< HEAD
This repository contains scripts and tools for research.
...
This script pulls data from News API, processes it, and adds the newest articles to a data base containing all the news data we have collected so far. The R script relies on **packages** with functions to do the following with relative ease:
=======
We are developing a data pipeline to collect headlines scraped from news sites using [**News API**](https://newsapi.org/). We want to explore the way different news sites go about alerting their followers and spreading information. A few informal questions:
...
<<<<<<< HEAD
This batch file overwrites the research sqlite data base with the most current gate-keeper data base.  Right now I have this running 2-4 times a day - less frequently so the research data base's bandwidth can be used towards answering questions and making discoveries.
=======
